...
- Add a new config-file options `auto_unfreeze`. If set, the bot will try to automatically unfreeze the account in case we get a frozen-message at the login. Don't enable this on servers running more than 5 bots, as you will get locked out by Funcom if you unlock more than 5 accounts per 24h.
