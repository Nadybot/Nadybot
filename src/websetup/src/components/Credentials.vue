<template>
  <form class="pt-6 w-50 mx-auto">
    ...
    <v-select
      :error-messages="dimension.errorMessage.value"
      label="Dimension"
      :items="dimensions"
      item-title="name"
      item-value="value"
      v-model="dimension.value.value"
    ></v-select>
...
const { validate, values } = useForm({
  validationSchema: {
    ...
    password(value: string | null) {
      ...
      else if (value != null && value.length > 20) return 'Password must not exceed 20 characters'
