 ...
 * @author Keex-1 (RK5)
 * @author Keltias (RK5)
 * @author TinkeringIdiot
 * @author Tradias (RK5)
 */
class SARechargeInfo {
	/**
	...
	public static function fromFastAttack(int $currentSkill, float $weaponAttack): self {
		...
		$skillToCap = (int)floor(($weaponAttack * 1_600) - ($hardCapTime + 1) * 100 + 1);
	...
	public static function fromAimedShot(int $currentSkill, float $weaponAttack, float $weaponRecharge): self {
		...
		$hardCapTime = (int)floor($weaponAttack + 10);
		$skillToCap = (int)floor($weaponRecharge * 4_000 / 3 + $weaponAttack * 100 / 3 - ($hardCapTime + 1) * 100 / 3 + 1);
