...
class DiscordGatewayController extends ModuleInstance {
	...
	public function connect(): void {
		...
		EventLoop::queue($this->connectToGateway(...));
	}
	...
	public function processDiscordGuildMessages(DiscordGatewayEvent $event): void {
		...
		EventLoop::queue($this->registerEmojis(...), $guild);
		EventLoop::queue($this->readAndCacheGuildInvites(...), $guild);
	...
	public function listDiscordEvents(
		...
		usort(
			$allEvents,
			static function (DiscordScheduledEvent $a, DiscordScheduledEvent $b): int {
	...
	private function removeFromVoice(string $userId): ?VoiceState {
		...
		$this->guilds[$oldState->guild_id]->voice_states = array_values(
			array_filter(
				...
				static function (VoiceState $state) use ($oldState): bool {
	...
	private function renderGuild(CmdContext $context, Guild $guild): string {
		...
		usort(
			$channels,
			static function (DiscordChannel $a, DiscordChannel $b): int {
		...
		do {
			...
			try {
				...
				while (null !== ($message = $connection->receive())) {
					...
					EventLoop::queue($this->processWebsocketMessage(...), $payload);
				}
