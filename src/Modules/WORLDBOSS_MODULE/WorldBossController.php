...
class WorldBossController extends ModuleInstance {
	...
	public const BOSS_DATA = [
		self::TARA => [
			...
			self::IMMORTAL => 30*60,
			self::COORDS => [2_092, 3_822, 505],
			self::AOU => 148,
		],
		self::REAPER => [
			self::INTERVAL => 9*3_600,
			self::IMMORTAL => 15*60,
			self::COORDS => [1_760, 2_840, 595],
			self::AOU => 746,
		],
		self::LOREN => [
			self::INTERVAL => 9*3_600,
		...
		self::VIZARESH => [
			self::INTERVAL => 17*3_600,
			self::IMMORTAL => 420,
			self::COORDS => [299, 28, 4_328],
			self::AOU => 615,
		],
		self::FATHER_TIME => [
			self::INTERVAL => 9*3_600,
		...
		self::ZAAL => [
			self::IMMORTAL => 15*60,
			self::INTERVAL2 => 6*3_600,
			self::CHANCE => 75,
			self::COORDS => [1_730, 1_200, 610],
			self::AOU => 730,
		],
		self::CERUBIN => [
			self::IMMORTAL => 15*60,
			self::INTERVAL2 => 9*3_600,
			self::CHANCE => 85,
			self::COORDS => [2_100, 280, 505],
			self::AOU => 730,
		],
		self::TAM => [
			self::IMMORTAL => 15*60,
			self::INTERVAL2 => 6*3_600,
			self::CHANCE => 60,
			self::COORDS => [1_130, 1_530, 795],
			self::AOU => 730,
		],
		self::ATMA => [
			self::IMMORTAL => 15*60,
			self::INTERVAL2 => 3*3_600,
			self::CHANCE => 30,
			self::COORDS => [1_900, 3_000, 650],
			self::AOU => 730,
		],
		self::ABMOUTH => [
			self::IMMORTAL => 15*60,
			self::COORDS => [3_150, 1_550, 556],
			self::AOU => 730,
		],
		self::DESERT_RIDER => [
			self::IMMORTAL => 15*60,
			self::COORDS => [2_230, 1_580, 565],
			self::AOU => 769,
		],
	];
	...
	public function formatWorldBossMessage(WorldBossTimer $timer, bool $short=true, bool $startpage=false): string {
		...
		if (isset($timer->next_spawn) && time() < $timer->next_spawn) {
			...
			$days = (int)floor((time() - $timer->spawn) / (24*3_600));
	...
	protected function addNextDates(array $timers): array {
		...
		foreach ($timers as $timer) {
			...
			if ($timer->next_killable <= time() && !isset($timer->timer)) {
				if (time() - $timer->next_killable <= 48 *3_600) {
					$newTimers []= $timer;
				}
				continue;
			}
